    ...
    <div class="page-content read container-fluid">
        <div class="row">
            ...
            <div class="col-md-6">
                <!-- Attributes -->
                <h4>Аттрибуты</h4>
                <form action="{{ route('attribute.add', ['id'=>$dataTypeContent->id]) }}" method="POST">
                    @csrf
                    <input type="text" name="name" placeholder="Аттрибут">
                    <input type="text" name="value" placeholder="Значение">
                    <button type="submit">Добавить</button>
                </form>
                <br>
                @foreach ($dataTypeContent->attributes as $attribute)
                    <p><strong>{{$attribute->name}}</strong>: {{$attribute->value}} <a href="{{ route('attribute.delete', ['id'=>$attribute->id]) }}"><span>(X)</span></a></p>
                @endforeach
            </div>
        </div>
    </div>
